...
class Lab4Spec(lab4: Lab4Like) extends AnyFlatSpec {
...

  {
    ...
    "typeof" should "handle numbers" in {
    assert(TNumber == typeof(Map(), N(3155.0)))
}
    it should "handle booleans" in {
      assert(TBool == typeof(Map(), B(true)))
      assert(TBool == typeof(Map(), B(false)))
    }
    it should "hanlde negation" in {
      assert(TNumber == typeof(Map(), Unary(Neg, N(3155))))
    }
    it should " handle not" in {
      assert(TBool == typeof(Map(), Unary(Not, B(true))))
      assert(TBool == typeof(Map(), Unary(Not, B(false))))
    }
    // it should "handle if" in {
    //      true ? 3155 : 2270
    // }
